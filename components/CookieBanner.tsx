    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ background: 'var(--navy)', padding: '20px 28px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: 600, color: '#fff', letterSpacing: '-0.01em' }}>
            Datenschutz & Cookies
          </span>
        </div>
        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '12px', color: 'var(--accent)', fontWeight: 500, padding: 0, display: 'flex', alignItems: 'center', gap: '4px', marginBottom: showDetail ? '20px' : '0', fontFamily: 'var(--font-sans)' }}
          >
        ...
        <div style={{ padding: '0 28px 24px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          {showDetail ? (
            <>
              <button onClick={saveSelection} style={{ flex: 1, minWidth: '140px', background: 'var(--navy)', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-sans)', transition: 'background 0.2s' }}>
                Auswahl speichern
              </button>
              <button onClick={acceptAll} style={{ flex: 1, minWidth: '140px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-sans)', transition: 'background 0.2s' }}>
                Alle akzeptieren
              </button>
            </>
          ) : (
            <>
              <button onClick={acceptNecessary} style={{ flex: '0 0 auto', background: 'transparent', color: 'var(--muted)', border: '1px solid var(--border2)', borderRadius: '8px', padding: '11px 20px', fontSize: '13px', fontWeight: 400, cursor: 'pointer', fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap' }}>
                Nur notwendige
              </button>
              <button onClick={acceptAll} style={{ flex: 1, background: 'var(--navy)', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-sans)', transition: 'background 0.2s' }}>
                Alle akzeptieren
              </button>
            </>
          )}
